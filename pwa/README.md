...
The service worker immediately caches only the app shell, the optional-pack manifest, and the shared loader. Large translator dictionaries and the campaign-search index are independently fetched, byte-hash verified, schema-validated, and cached only when the corresponding feature first needs them. A failed or corrupt replacement preserves the last known-good pack; each feature exposes a removal control, and cached packs remain available offline after validation.
