...
The service worker immediately caches the app shell, including the small party-funds fallback, plus the optional-pack manifest and shared loader. Party funds are served from `data/party-funds.json` through the versioned data cache, refreshing from the network first and falling back to the validated cached copy offline. Large translator dictionaries and the campaign-search index are independently fetched, byte-hash verified, schema-validated, and cached only when the corresponding feature first needs them. A failed or corrupt replacement preserves the last known-good pack; each feature exposes a removal control, and cached packs remain available offline after validation.

`optional-packs.json` is generated by `build-data.ps1` from the exact served bytes. Its content-addressed entries include the pack kind, schema, SHA-256, byte size, record/page count, and validation metadata. Do not edit it manually. The install shell never requests the approximately 10.1 MB optional payload.
