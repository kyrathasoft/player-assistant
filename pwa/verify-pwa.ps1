...
Assert-Condition -Condition ($appScript.Contains("'/login'") -and $appScript.Contains("'/session'") -and $appScript.Contains("'/xp'") -and $appScript.Contains("'/word-counts'") -and $appScript.Contains("'/presence'") -and $appScript.Contains("'/logout'")) -Message 'Character authentication, XP, word-count, and presence routes are incomplete.'
...
Assert-Condition -Condition ($dungeonMasterDashboardStyle.Success -and $dungeonMasterDashboardStyle.Groups['body'].Value.Contains('width: 192px;') -and $dungeonMasterDashboardStyle.Groups['body'].Value.Contains('height: 192px;')) -Message 'The Dungeon Master token in the XP card must be displayed at 192 pixels.'
...
Assert-Condition -Condition ($html.Contains('href="styles.css?v=23"') -and $html.Contains('src="app.js?v=23"') -and $serviceWorker.Contains("'./styles.css?v=23'") -and $serviceWorker.Contains("'./app.js?v=23'")) -Message 'The PWA shell must use cache-busting stylesheet and application-script URLs.'
