...
& "<skill-directory>\scripts\count-obsidian-publish-words.ps1" `
  ...
  -HistoryFile 'C:\repos\player-assistant\obsidian-wiki-word-count.md' `
  -LocalPostsRoot 'C:\repos\player-assistant\Release\Posts'
```
...
6. After a zero-failure run, append exactly one new bullet to `C:\repos\player-assistant\obsidian-wiki-word-count.md` matching the existing format: `- As of M/d/yyyy, the wiki contained N words; total IC words: I; total OOC words: O`. The bundled script does this automatically through `-HistoryFile`; never overwrite or rewrite existing entries.
