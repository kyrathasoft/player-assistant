...
4. Read every `*.html` and `*.md` file beneath `Release\Posts\IC` and `Release\Posts\OOC`, calculate one visible-content total for each directory, and exclude `.bak-*` duplicates plus non-HTML/Markdown state files. Always include `game-intro.html`/`.md` and `the-cast.html`/`.md` in the OOC total and in OOC comparisons; never exclude them because they are not thread pages. Reject an IC file containing numbered `msgpage` navigation because it is a paginated subset; refresh that thread from its RPOL show-all URL before counting.
5. For IC and OOC HTML thread pages, count only visible `messagebody` content. For Markdown exports, remove message-number and author metadata lines, then count the remaining visible content. For non-thread OOC pages, fall back to the visible page `content` section, then to the document body for locally exported standalone HTML. Exclude surrounding RPOL chrome, HTML/Markdown syntax, URLs, and RPOL edit/update notices.
