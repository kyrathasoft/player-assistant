...
- [x] Constrain remote broker-backup filenames before any SCP or local path construction.
  ...
  - [x] Added malformed-separator, traversal, rooted-path, confusable-name, extension, and approved-root regression coverage.
...
- [ ] Add measurable resource budgets.
  - [ ] Partial foundation: central resource-budgets.json and policy enforcement cover latency, growth, retention, startup, polling, optional packs, and diagnostics with desktop, PWA, and broker boundary tests.
...
- [ ] Add a versioned release-state schema and compatibility verifier.
  - [ ] Partial foundation: Version 1 release-state records and fail-closed compatibility verification enforce required fields, UTC timestamps, monotonic generations, and transition rules.
...
- [ ] Add structured correlation across desktop, broker, PWA, deployment, and scheduled-monitor operations.
  - [ ] Partial foundation: sanitized GUID correlation contexts and redaction contracts are available for desktop logs, HTTP headers, transaction state, and monitor/PWA boundaries.
...
- [ ] Make clock and timezone handling explicit at every date-sensitive boundary.
  - [ ] Partial foundation: UTC persistence boundaries and injected clocks reject ambiguous DST wall times and non-UTC signed/persisted timestamps.
...
- [ ] Add durable startup recovery for interrupted local desktop transactions.
  - [ ] Partial foundation: startup discovers transaction journals before settings load and fails closed on unverifiable promoted state while retaining journal evidence.
