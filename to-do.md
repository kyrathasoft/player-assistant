...
- [x] Continue Orcish work with a new quality pass outside the completed backlog.
  ...
  - Added a native `Show > Translator` WinForms view with English-to-Orcish default mode, an `Orcish to English` direction toggle, multiline input and read-only output, unknown-word preservation in both directions, background translation work, and no web-only hyperlinks or download controls.
